...
class AuthService:
    ...
    def register_user(self, db: Session, name: str, email: str, password: str) -> User:
        ...
        if not tier_obj:
            raise HTTPException(
                ...
                detail="System configuration error: free tier not found",
            )
    ...
    def verify_email_token(self, db: Session, token_str: str):
        ...
        if not db_token:
            raise HTTPException(
                ...
                detail="Verification token not recognized",
            )
    ...
    def request_password_reset(self, db: Session, email: str):
        ...
        jti = self.token.create_jti()
        reset_token = self.token.sign_forgot_password_token(
            {"email": normalized_email}, jti
        )
    ...
    def reset_password(self, db: Session, token_str: str, new_password: str):
        ...
        if not db_token:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Reset token not recognized",
            )
    ...
    def logout(self, db: Session, request: Request, response: Response):
        ...
        if raw_refresh:
            try:
                ...
                logger.warning(
                    "Failed to revoke refresh token during logout", exc_info=True
                )
