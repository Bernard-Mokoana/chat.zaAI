...
class TestTokenUtils:
    ...
    def test_token_init_missing_env_vars(self):
        with patch.dict("os.environ", clear=True):
            with pytest.raises(
                ValueError, match="JWT_SECRET environment variable is required"
            ):
                Token()
    ...
    def test_sign_and_decode_specialized_tokens(self):
        ...
        decoded_pwd = self.token_util.decode_forgot_password_verification_token(
            pwd_token
        )
    ...
    def test_rotate_refresh_token(self, mock_persist):
        ...
        new_access = self.token_util.rotate_refresh_token(
            db=self.mock_db, user=self.user, request=None, response=mock_response
        )
