...
class TestTokenBucket:
    def test_bucket_initialization_invalid_args(self):
        ...
        with pytest.raises(
            ValueError, match="refill_interval_seconds must be positive"
        ):
    ...
    def test_refill_logic(self, mock_monotonic):
        mock_monotonic.return_value = 1000.0
        bucket = TokenBucket(
            max_tokens=10, refill_tokens=5, refill_interval_seconds=10.0
        )
...
class TestCleanupLoop:
    ...
    async def test_cleanup_loop_execution(self, mock_sleep):
        ...
        try:
            await cleanup_loop(
                store=store, interval_seconds=10.0, max_idle_seconds=60.0
            )
