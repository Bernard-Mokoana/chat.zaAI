...
class TestAuthService:
    ...
    def test_register_user_success(self):
        ...
        self.mock_token_util.sign_email_verification_token.return_value = (
            "mocked-verify-token"
        )

        with patch(
            "backend.server.src.services.auth_services.send_email_verification"
        ) as mock_send_email:
            user = self.auth_service.register_user(
                db=self.db,
                name="Bernard Mokoana",
                email="bernardmokoana@gmail.com",
                password="111123!Example",
            )
            ...
            mock_send_email.assert_called_once_with(
                "bernardmokoana@gmail.com", "mocked-verify-token"
            )
    ...
    def test_register_user_invalid_email_format(self):
        with pytest.raises(HTTPException) as exc_info:
            self.auth_service.register_user(
                self.db, "Bernard", "invalid-email-format", "password123"
            )
        assert exc_info.value.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_register_user_email_already_registered(self):
        self.db.query.return_value.filter.return_value.first.return_value = User(
            id=1, email="bernardmokoana@gmail.com"
        )

        with pytest.raises(HTTPException) as exc_info:
            self.auth_service.register_user(
                self.db, "Bernard", "bernardmokoana@gmail.com", "password123"
            )
    ...
    def test_register_user_password_too_short(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            self.auth_service.register_user(
                self.db, "Bernard", "test@example.com", "short"
            )
        assert exc_info.value.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_authenticate_user_invalid_email(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            self.auth_service.authenticate_user(
                db=self.db, email="bernarddira@gmail.com", password="password@1235"
            )
    ...
    def test_login_success(self):
        ...
        result = self.auth_service.login(
            self.db, mock_user, mock_request, mock_response
        )

        assert result["access_token"] == "access-token-string"
        assert result["user"]["id"] == "user-uuid"
        self.mock_token_util.set_refresh_cookie.assert_called_once_with(
            mock_response, "refresh-token-string"
        )
    ...
    def test_verify_email_token_expired(self):
        ...
        with patch(
            "backend.server.src.services.auth_services.datetime"
        ) as mock_datetime:
    ...
    def test_request_password_reset_user_not_found(self):
        ...
        result = self.auth_service.request_password_reset(
            self.db, "nonexistent@example.com"
        )
    ...
    def test_request_password_reset_success(self):
        ...
        self.mock_token_util.sign_forgot_password_token.return_value = (
            "reset-token-string"
        )

        with patch(
            "backend.server.src.services.auth_services.send_password_reset_email"
        ) as mock_email:
            result = self.auth_service.request_password_reset(
                self.db, "bernarddira@gmail.com"
            )

            self.mock_token_util.invalidate_reset_password_tokens.assert_called_once_with(
                self.db, mock_user.id
            )
            mock_email.assert_called_once_with(
                "bernarddira@gmail.com", "reset-token-string"
            )
            assert "link has been sent" in result["message"]
    ...
    def test_logout_clears_cookie_and_revokes_token(self):
        ...
        self.mock_token_util.decode_refresh_token.return_value = {
            "id": "user-uuid",
            "jti": "jti-123",
        }
