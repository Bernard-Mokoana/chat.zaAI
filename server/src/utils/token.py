...
class Token:
    ...
    def sign_access_token(self, user: dict) -> str:
        try:
            minutes = int(self.expires_in[:-1]) if self.expires_in.endswith("m") else int(self.expires_in)
    ...
    async def rotate_refresh_token(self, db: Session, user: dict, request: Optional[Request], response: Optional[Response]) -> str:
